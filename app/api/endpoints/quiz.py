...
async def create_quiz(quiz: QuizCreate,
    ...
    db_quiz = await service.create_quiz(quiz=quiz, current_user_id=current_user.user_id, company_service=company_service)
    return Result(result=QuizBase(**db_quiz))
...
async def update_quiz(quiz: QuizUpdate,
    ...
    result = await service.update_quiz(current_user_id=current_user.user_id, quiz=quiz, company_id=company_id, quiz_id=quiz_id,company_service=company_service)
    return Result[QuizBase](result=result, message="Quiz has been updated")


@router.delete("/{quiz_id}/", status_code=200, response_model=Result, response_description="Quiz delete")
async def delete_quiz(company_id: int,
                    quiz_id:int,
                    service: QuizService = Depends(get_quiz_service),
                    company_service: CompanyService = Depends(get_company_service),
                    current_user: UserResponse = Depends(get_current_user)) -> Result:
    result= await service.delete_quiz(company_id=company_id, current_user_id=current_user.user_id, company_service=company_service, quiz_id=quiz_id)
    return Result(result=result, message="Quiz deleted successfully")
