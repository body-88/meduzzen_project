...
@router.post("/{quiz_id}/company/{company_id}", response_model=Result[QuestionBase], status_code=200, response_description="Question created")
...
@router.put("/{question_id}", response_model=Result[QuestionBase], status_code=200, response_description="Question updated")
async def update_question(question: QuestionUpdate,
    ...
    result = await service.update_question(current_user_id=current_user.user_id, question=question,
                                        ...
                                        quiz_service=quiz_service)
    return Result[QuestionBase](result=result, message="Question has been updated")


@router.delete("/{question_id}/", status_code=200, response_model=Result, response_description="Question delete")
async def delete_question(question_id: int,
    ...
    result=await service.delete_question(current_user_id=current_user.user_id,
                                        ...
                                        quiz_service=quiz_service)
    return Result(result=result, message="Question deleted successfully")